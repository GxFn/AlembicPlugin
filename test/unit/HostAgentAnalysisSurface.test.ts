...
} from '../../lib/recipe-generation/generate/HostAgentAnalysisSurface.js';
import {
...
function makePacket(): HostAgentAnalysisPacket {
  return {
    packetId: 'host-packet-1',
...
): HostAgentAnalysisPacket['units'][number] {
  return {
    unitId,
    key: {
      dimensionId,
...
describe('HostAgentAnalysisSurface', () => {
  it('keeps IDEAgent surface builders as R1 compatibility aliases', () => {
    expect(buildIDEAgentAnalysisSurface).toBe(buildHostAgentAnalysisSurface);
    expect(buildIDEAgentAnalysisProgressBackfill).toBe(buildHostAgentAnalysisProgressBackfill);
  });

  it('projects Core packet summary, next units, retrieval, and progress without private schema', () => {
    const surface = buildHostAgentAnalysisSurface(makePacket(), {
      progressOverrides: [
        {
          unitId: 'unit-1',
          status: 'completed',
    ...
    expect(surface.packetSummary.packetId).toBe('host-packet-1');
  ...
  it('projects ProjectScope-qualified source identities into Codex read sets', () => {
    ...
    const surface = buildHostAgentAnalysisSurface(packet);
  ...
  it('builds checkpoint backfill for completed, skipped, rejected, and remaining units', () => {
    const backfill = buildHostAgentAnalysisProgressBackfill({
