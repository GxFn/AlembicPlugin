...
router.post('/quality/refresh-all', async (_req: Request, res: Response) => {
  if (!rejectUnlessConfirmed(_req, res, 'knowledge quality refresh-all')) {
    return;
  }
  const container = getServiceContainer();
  const knowledgeService = container.get('knowledgeService');
...
router.delete('/:id', async (req: Request, res: Response) => {
  if (!rejectUnlessConfirmed(req, res, 'knowledge delete')) {
    return;
  }
  const id = String(req.params.id);
  const container = getServiceContainer();
 ...
 * 进化 (active → evolving)
 */
router.patch('/:id/evolve', async (req: Request, res: Response) => {
  const id = String(req.params.id);
  const container = getServiceContainer();
  const knowledgeService = container.get('knowledgeService');
  const context = getContext(req);
 ...
 */
router.patch('/:id/decay', async (req: Request, res: Response) => {
  const id = String(req.params.id);
  const container = getServiceContainer();
  const knowledgeService = container.get('knowledgeService');
  const context = getContext(req);
...
router.post('/batch-publish', validate(BatchPublishBody), async (req: Request, res: Response) => {
  if (!rejectUnlessConfirmed(req, res, 'knowledge batch-publish')) {
    return;
  }
  const { ids } = req.body;
 ...
 * 批量删除知识条目
 */
router.post('/batch-delete', validate(BatchDeleteBody), async (req: Request, res: Response) => {
  if (!rejectUnlessConfirmed(req, res, 'knowledge batch-delete')) {
    return;
  }
  const { ids } = req.body;
...
router.post(
  ...
  async (req: Request, res: Response) => {
    if (!rejectUnlessConfirmed(req, res, 'knowledge batch-deprecate')) {
      return;
    }
