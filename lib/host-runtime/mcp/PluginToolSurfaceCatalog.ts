...
function catalogEntry(entry: PluginToolSurfaceEntry): PluginToolSurfaceEntry {
  return entry;
}

// Plugin-visible tool metadata lives here so annotations, gateway mappings, and
// handler ownership cannot drift across the list and router.
export const PLUGIN_TOOL_SURFACE_CATALOG = {
  // MTC-4: merged alembic_health + alembic_mcp_status + alembic_codex_diagnostics.
  // Listed once and handled by the request-scoped Host server.
