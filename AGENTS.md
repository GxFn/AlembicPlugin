...
- 只允许 claim / finish `AlembicPlugin` 目标任务；不得代领、处理、验证或总结其它窗口任务。
...
- `AlembicPlugin` 不负责 `AlembicTest` 自动化、真实测试验证或测试窗口任务；`AlembicTest` 下一跳默认由总控调起，本窗口不得创建、处理或代验 `AlembicTest` heartbeat，除非当前总控计划和 finish JSON 同时显式允许 courier。

## 文档存储提示

- 新建长期迁移、计划、验收、扫描、边界和跨仓库任务文档时，统一写到 workspace 根目录的 `docs/AlembicPlugin/`，不要散落到各子仓库或 workspace `docs/` 根层级。
- AlembicCore 迁移手册、公开 API 边界、阶段验收、外层接入和删除任务统一写到 `docs/AlembicCore/`；本仓库只执行其中分配给 `AlembicPlugin` 窗口的任务。
...
- 本仓库不承载独立 Agent runtime、AI provider runtime、Agent Tool V2 runtime；不得重新引入 `@alembic/agent`、`#agent/*`、`#tools/*`、`#external/ai/*`、`lib/agent/**`、`lib/tools/**` 或 `lib/external/ai/**`，除非用户和新的总控文档明确改变边界。
...
- 正式源码：`lib/`、`bin/`、`config/`。
- 正式脚本：`scripts/`。
- 正式文档：`docs/`；仓库内 `docs/` 仅放随源码长期维护的产品/发布文档，跨仓库迁移和验收文档写入 workspace 的 `docs/AlembicPlugin/`。
